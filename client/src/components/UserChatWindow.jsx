...
const UserChatWindow = () => {
  ...
  return (
    Object.keys(selectedUserConversation).length > 0 ? (
      <>
        <Box bg={'#131313'} minH={"100vh"} maxH={"100vh"}>
            ...
            <Flex alignItems={"center"} justifyContent={"space-between"} pl={10} pr={5} py={3}>
              ...
              <Flex justifyContent={"center"} flexDir={"column"}>
                  <Flex alignItems={"center"} gap={3}>
                      ...
                      <Flex justifyContent={"center"} flexDirection={"column"} w={"180px"}>
                          <Text color={"#fff"} fontSize={"17px"}>{selectedUserConversation.conversationName}</Text>
                          {userTyping && <Flex color={"#666"} fontSize={"13px"} gap={1} alignItems={"center"}>
              ...
              <Flex>
                <IconButton {...BUTTON_STYLE} bg={"#131313"} borderRadius={"full"} onClick={onOpen} icon={<SlOptionsVertical/>}/>
              </Flex>
            </Flex>
                  ...
                  messages.map((m, i) => (
                    <div style={{ display: "flex", alignItems: "flex-end" }} key={m._id}>
                      ...
                      >
                        ...
                        >
                          {/* Reply To  */}
                          {/* <Flex alignItems={"center"} gap={2} overflow={"hidden"} bg={m?.sender?._id === user._id ? "#f0f4f9" : "#333"} 
                            py={1} px={3} borderRadius={"10px 10px 5px 5px"} fontSize={"15px"} mb={1}
                            color={m?.sender?._id === user._id ? "#333444" : "#666"} 
                          >
                            <Text color={m?.sender?._id === user._id ? "#fff" : "#999"} className={m?.sender?._id === user._id ? "header-logo-text" : ""}>@mayur</Text> 
                          ...
                          <Flex align={"center"} justifyContent={"space-between"} gap={"10px"}>
                            ...
                            {!m.isDeleted ? (
                              ...
                              <Flex alignItems={"center"}  gap={2} fontWeight={"300"} opacity={.7}>
            ...
            <Box display={"flex"} justifyContent={"center"} py={3} position={"relative"}>
                {/* IMAGE SEND SECTION */}
                {uploadImage && <Box pos={"absolute"} bottom={3} left={5} bg={"#191919"} overflow={"hidden"} boxShadow='sm' borderRadius={"xl"} w={"400px"} height={"320px"} border={'1px solid #191919'} zIndex={1}>
                  <Box w={"full"} height={"260px"} bg={"#131313"} borderRadius={"xl"} overflow={"hidden"}>
                    <Box pos={"absolute"} right={3} top={3} zIndex={2} fontSize={"30px"} color={"#191919"} cursor={"pointer"} onClick={() => setUploadImage("")}><IoCloseCircle/></Box>
                  ...
                  <Flex mt={3} pr={2}>
                    ...
                    <IconButton onClick={sendMessage} borderRadius={'full'} {...BUTTON_STYLE} _active={{opacity: 0.7}} icon={<BiSolidSend color="#FFF" fontSize={'20px'} />} isDisabled={uploadImage === ""}/>
                  </Flex>
                </Box>}
                    ...
                    <Flex alignItems={'end'} border={'1px solid #333'} gap={1} borderRadius={'30px'} p={1} w={'50vw'} transition={'width .5s ease-in-out'} minH={'50px'}>
                        <Box>
                          ...
                          <Tooltip label={"Attachment"} {...TOOLTIP_STYLE}>
                            <IconButton onClick={() => fileRef.current.click()} borderRadius={'full'} {...BUTTON_STYLE} _active={{opacity: 0.7}} icon={<ImAttachment color="#fff" fontSize={'20px'} />}/>
                          </Tooltip>
                        </Box>
                        ...
                        <IconButton borderRadius={'full'} {...BUTTON_STYLE} _active={{opacity: 0.7}} icon={<BiSolidSend color="#fff" fontSize={'20px'} />} isDisabled={newMessage === ""}/>
                    </Flex>
                </form>
            </Box>
        </Box>
      ...
      <Flex bg={'#131313'} minH={"100vh"} maxH={"100vh"} alignItems={"center"} justifyContent={"center"} px={40}>
        <Text fontSize={"80px"} fontWeight={"500"} color={"#222"} >Welcome! Select a user to begin your chat journey.</Text>
      </Flex> 
    )
)}
