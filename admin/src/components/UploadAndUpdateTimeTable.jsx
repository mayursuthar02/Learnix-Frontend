...
import { useRecoilValue } from "recoil";
  ...
  const [formData, setFormData] = useState({
    ...
    const divisionsList = [
      {title: "Select", value: ""},
      {title: "All Divisions", value: "all divisions"},
      {title: "Division A", value: "division A"},
      {title: "Division B", value: "division B"},
      {title: "Division C", value: "division C"},
      {title: "Division D", value: "division D"},
      {title: "Division E", value: "division E"},
      {title: "Division F", value: "division F"},
      {title: "Division G", value: "division G"},
    ]
    // Function
    ...
    // Get Single Resource
    useEffect(()=> {
        setFormData({ title: "", division: "", semester: "", resourceLink: "", description: "", file: "", fileName: null });
        if (modelMode === "update" && resourceIdForUpdate) {
            const fetchResource = async () => {
              ...
              try {
                ...
                console.log(data);
                const { title, division, semester, resourceLink, description } = data.resource;
                setFormData({ title, division, semester, resourceLink, description, file: "", fileName: null });
              } catch (error) {
    ...
    const handleResourceAction = async () => {
        ...
        try {
          const { title, division, semester, file, resourceLink, description } = formData;
      
          if (!title || !division || !semester) {
            showToast("Error", "Title, Semester, and division fields are required!", "error");
            return;
          }

          const resourceData = new FormData();
          resourceData.append("title", title);
          resourceData.append("division", division);
          ...
          <ModalContent>
            ...
            <ModalBody pb={6}>
              ...
              <Grid gridTemplateColumns={'repeat(2, 1fr)'} gap={5} mt={5}> 
                <FormControl isRequired>
                  <FormLabel color={"#1f1f1f"} fontSize={"16px"} fontWeight={"500"}>Division</FormLabel>
                  <Select 
                  value={formData.division}
                  onChange={(e) => handleInputChange("division", e.target.value)}
                  isDisabled={isLoading.resource}
                  >
                    {divisionsList.map((type,i) => (
              ...
              <FormControl mt={5} isDisabled={isLoading.resource}>
                ...
                <Flex alignItems={'center'} justifyContent={'center'} border={'1px solid #ededed'} borderRadius={'4px'} py={5} cursor={"pointer"} _hover={{bg: "#efefef", borderColor: "#eee"}} transition={"background .3s ease"} gap={2} onClick={()=>fileRef.current.click()}>
                    ...
                    <Text fontSize={"17px"} color={"#7b899d"}>{formData.file.name || "Select Time Table Resource From Browser"}</Text>
                </Flex>
              </FormControl>
  ...
  export default UploadAndUpdateTimeTable;